...
export const Registration: React.FC = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // TFHE public keys are too large to store as Solidity strings within a
      // local Hardhat block gas limit. The full key remains in localStorage
      // and FHE-API (where encryption uses it); the contract records a stable
      // cryptographic commitment instead.
      const fhePublicKeyCommitment = ethers.utils.keccak256(
        ethers.utils.toUtf8Bytes(keys.publicKey)
      );

      const tx = await userRegistry.registerUser(
        wallet.address,
        fhePublicKeyCommitment,
        "serverKey"
      );
