...
import chineseFontUrl from '../../assets/fonts/DroidSansFallbackFull.ttf';

const { Paragraph, Title, Text } = Typography;
const { Option } = Select;

let chineseFontBase64Promise: Promise<string> | null = null;

const loadChineseFontBase64 = async () => {
  if (!chineseFontBase64Promise) {
    chineseFontBase64Promise = fetch(chineseFontUrl)
      .then(async (response) => {
        if (!response.ok) {
          throw new Error(`Failed to load Chinese PDF font: ${response.status}`);
        }
        ...
        // Process in chunks so this large CJK font stays below the browser's
  ...
  return chineseFontBase64Promise;
};
...
export const TaskResults: React.FC = () => {
    ...
    try {
      ...
      const formatTasks = (tasks: any[]): Task[] => tasks.map((task: any) => ({
        ...
        status: task.isCompleted 
          ...
          : 'pending',
        // Solidity block.timestamp uses seconds; Date expects milliseconds.
        createdAt: parseInt(task.createdAt._hex, 16) * 1000,
  ...
  const generateCertificate = async () => {
    try {
      ...
      const doc = new jsPDF();
      const chineseFontBase64 = await loadChineseFontBase64();
      doc.addFileToVFS('DroidSansFallbackFull.ttf', chineseFontBase64);
      doc.addFont('DroidSansFallbackFull.ttf', 'DroidSansFallback', 'normal');
      doc.setFont('DroidSansFallback', 'normal');
      ...
      // 中文标签使用嵌入的 CJK 字体；任务编号、地址、计算结果和签名
      // 使用 jsPDF 内置 Helvetica，避免 CJK fallback 字体遗漏 ASCII 字形。
      const basicContent = [
        { label: '证明编号：', value: currentTask.id },
        { label: '银行地址：', value: currentTask.bankId },
        { label: '业务类型：', value: currentTask.businessType },
        { label: '资质结果：', value: currentTask.decryptedResult ?? '' },
        { label: '签发日期：', value: new Date().toLocaleDateString() }
      ];

      let yPosition = 40;
      basicContent.forEach(({ label, value }) => {
        doc.setFont('DroidSansFallback', 'normal');
        ...
        const valueLines = doc.splitTextToSize(
          String(value),
          Math.max(maxLineWidth - labelWidth, 20)
        );
        doc.text(valueLines, margin + labelWidth, yPosition);
        yPosition += valueLines.length * 8 + 2;
      });
