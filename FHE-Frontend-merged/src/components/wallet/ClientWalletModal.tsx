...
export const ClientWalletModal: React.FC = () => {
  ...
  const generateWallet = async () => {
    ...
    // Keep client funding independent from the bank implementation. This is
    // intentionally the same local-Hardhat flow used by the bank wallet.
    const provider = new ethers.providers.JsonRpcProvider('http://127.0.0.1:8545');
    const faucetWallet = provider.getSigner('0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266');
    const tx = await faucetWallet.sendTransaction({
      to: newWallet.address,
      value: ethers.utils.parseEther('10')
    });

    // Registration and task creation are enabled only after this confirmation.
    await tx.wait();
    ...
    messageApi.success('新客户端钱包生成成功，已获得 10 ETH 测试资金！');
  };
