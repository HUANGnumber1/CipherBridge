...
# 8) decryption-proof 的 guest 必须按“读一份 → 反序列化 → 立即释放”处理输入（7 处 env::read）
#    否则 7 份输入全部常驻(≈114MB) + bincode 反序列化时的临时块(64MiB) 会超过 guest 的 192MiB 上限
grep -c 'env::read' /root/Bisai/zkFHE-Decryption-Proof/decryption-proof/methods/guest/src/main.rs   # 应为 7
#    注意：本项改完 ZK2 仍未通过（卡在 bsk 反序列化的 64MiB 分配），详见 docs/WORK_LOG_2026-09-14.md
...
- ⚠️ **guest 内存硬上限 192MiB**：risc0 1.2 的 zkVM guest 地址空间上限为 `risc0-zkvm-platform::memory::GUEST_MAX_MEM = SYSTEM.start = 0x0C00_0000`（192MiB），堆从 guest ELF 的 `_end` 顶到该上限；本 demo 输入合计 ≈114MB（`std_bootstrapping_key` ≈6.19M×u64 ≈50MB + `fourier_bsk` ≈4.19M×c64 ≈64MB）。guest 已改为「读一份 → 反序列化校验 → 立即释放」，峰值内存 ≈130MB；**不要改回**“先把 7 份输入全读成 `Vec<u8>` 再逐个反序列化”（那样峰值 ≈210MB，必然触发 `memory allocation of 67108864 bytes failed`，详见第 5 节）；
- ⚠ host 内部会 `println!("{:?}")` 打印超大密钥结构体，**日志会涨到数百 MB（312MB 实测）属正常现象**；跑之前确认磁盘充足；
- guest 在 zkVM（r0vm 模拟 RV32）里执行 PBS + 解密 + `assert_eq!`，**这段是纯 CPU 计算，需要数分钟**，CPU 会长期 100%；
...
| 运行时 `Guest panicked: Out of memory! You have been using the default bump allocator which does not reclaim memory. Enable the heap-embedded-alloc feature`（decryption-proof，exit 101） | guest 输入总量 ≈114MB（`std_bootstrapping_key`≈50MB + `fourier_bsk`≈64MB），risc0 默认 bump 分配器不回收内存，历史峰值一超即崩 | guest `Cargo.toml` 的 risc0-zkvm 加 `'heap-embedded-alloc'`（配合下一条的 vendor 补丁，`tools/env_setup.sh` 已内置） |
| guest 打印 `memory allocation of 67108864 bytes failed`，紧接着 host 报 `panicked at host/src/main.rs:244: called Result::unwrap() on an Err value: Trap: IllegalInstruction(c0001073), pc: 0x...`（exit 101，且 host 内存充足、无 OOM killer 记录） | **guest 内 192MiB 地址空间被打满**（不是宿主内存不足）：旧写法把 7 份输入先全部读成 `Vec<u8>` 常驻(≈114MB) + bincode 反序列化时的 64MiB 连续块 + 已解析的 `std_bootstrapping_key` 结构体(≈50MB) ≈ 210MB > 192MiB；改成流式写法后峰值 ≈161MB，理论上应能容纳，但仍在该处失败（未决问题） | 用 guest 现有的「读一份 → 反序列化 → 立即释放」写法（`methods/guest/src/main.rs`：只保留后续计算要用的 `pbs_multiplication_ct` / `big_lwe_sk` / `cleartext_multiplication_result`，其余解析后立刻出作用域），峰值 ≈161MB。**注意：此项改完 ZK2 仍未通过**（执行推进到第 1 份输入 `std_bootstrapping_key` 的反序列化处仍申请 64MiB 失败），进展与待验证项见 `docs/WORK_LOG_2026-09-14.md` 第 4–5 节。注意：`heap-embedded-alloc` 只是让已分配内存可被回收，**不会**扩大 192MiB 硬上限 |
